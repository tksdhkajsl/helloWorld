...
//트리(Tree) 선(간선):Edge /형제자매: 부모가 같은 노드들(Sibling)/자식이 없음(Leaf/Terminal)
/* 특징
	- 부모-자식 관계를 가지는 계층적 구조를 나태내는 자료구조
	- 하나의 Root(부리)에서 시작하여 가지가 뻗어나가는 형태
	- 순환이 없는 그래프의 한 종류
	- 대표적인 비선형 자료구조
-	장점
		- 계층적인 데이터를 표현하기에 최적화 되어 있다.
		- 대체로 탐색과 정렬에 매우 효율적인 성능을 보인다.(이진 탐색 트리 계열)
		- 배열과 리스트의 장점을 둘 다 가지고 있다.(탐색이 빠르면서 삽입 삭제도 빠르고 크기도 동적이다.)
	-단점
		- 구조가 복잡하고 구현이 어려움
		- 데이터를 추가.삭제하는 과정이 복잡할 수 있다.(균형을 맞춰야할 때.)
  - 트리의 순회
	- 트리의 모든 노드를 한번씩 방문하는 방법
	- 깊이 우선 탐색 : 일단 아래쪽으로 가는 것을 우선(매우 자주 사용되는 방식)
		- 전위 순회 : 루트 -> 왼쪽 -> 오른쪽
		- 중위 순회 : 왼쪽 -> 루트 -> 오른쪽(순서대로)
		- 후위 순회 : 왼쪽 -> 오른쪽 -> 루트
